...
def db(request):
    ...
    return render_to_response('db.html', {'greetings': greetings}, context_instance = RequestContext(request))
...
def directions2(request):
    ...
    if form.is_valid():
        ...
        cursor.execute("select find_path(%s,%s)",[source,dest])
...
def info2(request):
    ...
    if form.is_valid():
        ...
        cursor = connection.cursor()
        cursor.execute("SELECT * from hello_stationinfo where sname = %s",[sname])
        infor = cursor.fetchone()
        cursor.execute("SELECT * from hello_station where sname =%s",[sname])
        inform = cursor.fetchall()
        cursor.execute("SELECT place from hello_places where sname =%s",[sname])
...
def nearest2(request):
    ...
    if form1.is_valid():
        ...
        cursor.execute("SELECT sname from hello_places where place =%s",[place])
...
def nearest3(request):
    ...
    if form2.is_valid():
        ...
        cursor = connection.cursor()
        cursor.execute("SELECT sname from hello_stationinfo where pincode =%s",[pin])
...
def review2(request):
    ...
    if form1.is_valid():
        ...
        cursor = connection.cursor()
        cursor.execute("SELECT sname,title,author,bodytext from hello_review where approved = 'Yes'")
        data = cursor.fetchall()
...
def review3(request):
    ...
    if form2.is_valid():
        ...
        cursor = connection.cursor()
        cursor.execute("INSERT INTO hello_review (sname,title,author,bodytext,timest,approved) VALUES (%s,%s,%s,%s,now(),'no')",[sname,title,author,bodytext])
