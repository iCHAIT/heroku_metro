...
class dirForm(forms.Form):
    ...
    cursor.execute("select sname,sname from hello_stationinfo")
...
class near1Form(forms.Form):
    cursor = connection.cursor()
    cursor.execute("select place,place from hello_places")
    CHOICES = cursor.fetchall()
...
class near2Form(forms.Form):
    cursor = connection.cursor()
    cursor.execute("select pincode,pincode from hello_stationinfo")
    CHOICES = cursor.fetchall()
